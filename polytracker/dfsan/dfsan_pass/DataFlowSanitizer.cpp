...
bool DataFlowSanitizer::doInitialization(Module &M) {
  ...
  InstPtrTy = PointerType::get(IntegerType::get(*Ctx, 8), 0); // any pointer
  ...
  //Type * DFSanLogTaintArgs[1] = {InstPtrTy};
  Type * DFSanLogTaintArgs[3] = {Type::getInt32Ty(*Ctx), Type::getInt32Ty(*Ctx), ShadowTy};
  //Type * DFSanLogTaintArgs[1] = {IntegerType::getInt32Ty(getGlobalContext())};
...
void DFSanVisitor::logTaintedOps(Instruction * InsertPoint, Value * Shadow) {
  int line = 0;
  int col = 0;
  const DebugLoc &location = InsertPoint->getDebugLoc();
  if (location) {
  ...
  if(PRINT_NAME) 
  {
    args[0] = IRB.CreateGlobalStringPtr(StringRef(InsertPoint->getOpcodeName()));
    args[1] = IRB.CreateGlobalStringPtr(StringRef(InsertPoint->getOpcodeName()));
  }
  else
  {
  ...
  args[2] = Shadow;
  CallInst *Call = IRB.CreateCall(DFSF.DFS.DFSanLogTaintFn, args);
  Call->addAttribute(AttributeList::ReturnIndex, Attribute::ZExt);
  Call->addParamAttr(0, Attribute::ZExt);
}
...
void DFSanVisitor::visitLoadInst(LoadInst &LI) {
  ...
  if (ClCombinePointerLabelsOnLoad) {
    ...
    //errs()<<LI.getOperand(0)->getName();
    //LI.getOperand(0)->print(errs());
    //errs()<<"\n";
    //std::cout << "666"<<std::endl;
    //llvm::outs << LI.getPointerOperand()->getName();
